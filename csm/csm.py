...
# Launch ClassicSimon. The .nvgt source lives in src/, but the assets (docks, lib, sounds)
# live here in csm/, so we run with cwd set to this folder: every cwd-relative path in the game
# (sounds/..., lib/..., docks/...) then resolves against csm/, while the script's own
# #include"includes/*.nvgt" resolves relative to src/. We run the console build nvgt.exe (not the
...
SCRIPT = os.path.normpath(os.path.join(HERE, "..", "src", "csm.nvgt"))
...
COMPILE_WAIT = 5
# Where a compile-error report is written: the csm/ folder next to this launcher, so it's always in the
...
def fail(message):
    ctypes.windll.user32.MessageBoxW(0, message, "ClassicSimon launcher", 0x10)
    sys.exit(1)


def sync_version():
    # build/version.txt is the single source of truth; mirror it into src/includes/version.nvgt (which
    # csm.nvgt glob-includes) before launch so the running game reports the current version. Only rewrite
    # when it changed; keep LF (this repo's convention). A missing/empty version.txt just leaves it alone.
    try:
    ...
    if not version:
        return
    line = 'string version = "%s";' % version + chr(10)
    try:
    ...
    try:
        ...
        fail("Failed to sync the version into version.nvgt: " + str(error))
...
if not os.path.isfile(SCRIPT):
    fail("Could not find the ClassicSimon script at:\n" + SCRIPT)
...
log_fd, log_path = tempfile.mkstemp(prefix="classicsimon_", suffix=".log")
...
try:
    ...
    try:
        ...
        pass
    fail("Failed to start ClassicSimon:\n" + str(error))
...
# surface the captured output to errors.txt (in the csm/ folder) and tell the user.
log_file.close()
if code != 0:
    try:
        ...
        details = ""
    body = details if details else "(ClassicSimon exited with code %d but printed no output)" % code
    try:
    ...
    try:
        ...
        pass
    fail("ClassicSimon failed to start (exit code %d).\nDetails written to:\n%s" % (code, ERRORS_PATH))
