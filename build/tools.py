...
SRC_DIR      = os.path.join(REPO_DIR, "src")                     # entry .nvgt lives here (src/csm.nvgt)
ASSETS_DIR   = os.path.join(REPO_DIR, NVGT_OUT)                  # asset folder (docks, lib, sounds); its name == NVGT_OUT (csm)
BUNDLE       = os.path.join(SRC_DIR, NVGT_OUT)                   # nvgt -c (run from src) produces this bundle folder
VERSION_NVGT = os.path.join(SRC_DIR, "includes", "version.nvgt") # mirror of build/version.txt, glob-included by csm.nvgt
ASSET_FOLDERS = ["docks", "lib", "sounds"]                      # copied into the bundle after compile (sounds ARE shipped for this game)

NVGT    = _tools["tools"]["nvgt"]  # shared across all the games; set to the legacy build in ~/.game_tools/tools.ini
SEVENZIP = _tools["tools"]["sevenzip"]
GH      = _tools["tools"]["gh"]

WIN_SOURCE   = os.path.join(REPO_DIR, "releases", "windows", f"{GAME}_windows_portable", NVGT_OUT)
ARCHIVE_DIR  = os.path.join(REPO_DIR, "releases", "archives")
ARCHIVE_NAME = f"{GAME}_windows_portable.7z"
ARCHIVE      = os.path.join(ARCHIVE_DIR, ARCHIVE_NAME)
RELEASE_DIR  = os.path.join(REPO_DIR, "releases", "windows", f"{GAME}_windows_portable")
...
def sync_version_file(version):
    ...
    # version (a compiled release has no build/version.txt beside it to read at runtime). This repo is
    # LF (see .gitattributes), so write LF.
    with open(VERSION_NVGT, "w", newline="", encoding="utf-8") as f:
        f.write('string version = "%s";\n' % version)
    print("Synced version %s into version.nvgt.\n" % version)
...
def run_release(skip_compile, skip_package, skip_release, skip_empty_release, interactive=True):
    ...
    if do_compile:
        ...
        # asset/document pragmas don't cover the src/asset split. lib merges with the runtime DLLs.
    ...
    if do_package:
        ...
        if not run_cmd([SEVENZIP, "a", "-t7z", ARCHIVE, WIN_SOURCE, "-mx=9", "-m0=LZMA2", "-md=64m", "-mfb=64", "-ms=on", "-mmt=12"]):
...
if __name__ == "__main__":
    ...
    if not args:
        ...
        if len(args) != 4:
            print("Error: expected 4 integer flags (skip_compile skip_package skip_release skip_empty_release; 0=ask, 1=do, 2=silent-skip).")
            sys.exit(2)
        try:
