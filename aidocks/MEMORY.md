...
- [Pack config (info.cmf)](project_pack_config.md) — per-pack timing tuning via csm/sounds/simons/<pack>/info.cmf (key=value, colon lists easy:medium:hard:insane); parsed by class cmf in main/parsers/game_parser.nvgt into the packinfo global; every value falls back to the game default (empty/missing file = unchanged).
