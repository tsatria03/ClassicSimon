...
**How to apply:** Use a unique/fresh filename for regenerated audio, or load with `allow_preloads=false` so the cache is bypassed. Static shipped sounds (the `csm/sounds/` tree) are fine — this only affects sounds whose bytes change under a fixed name. See [[project_audio_model]].
